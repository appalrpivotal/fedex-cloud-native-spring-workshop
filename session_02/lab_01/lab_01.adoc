...
. Change the package name to com.{yourlastname}. For example: com.womack
. Change the app name from demo to orders-service
. Add the following dependencies:
+
----
JPA, Rest Repositories, H2, Actuator
----
. Click 'Generate Project', and save the zip file to this directory $COURSE_HOME/session_02/lab_01
. Unzip the project
. Delete the .zip file, copy the manifest.yml file from this directory into the newly extracted project directory
. Add the following yaml stanza to a new file src/main/resources/bootstrap.yml (This is optional, but will be useful to view actuator endpoints later in the workshop)
+
```
---
security:
...
+
----
$ cd $COURSE_HOME/session_02/lab_01/orders-service
----
...
Using manifest file /Users/swomack/workspace-workshops/esi-cloud-native-spring-workshop/session_02/lab_01/orders-service/manifest.yml
...
Because our application knows nothing about any backing databases, and h2 is on the classpath, h2's in memory database will be used when this application starts up. In the next lab, we'll bind a backing database
